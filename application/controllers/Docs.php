...
class Docs extends CI_Controller{
	...
	private function _get_lib_source($file_name){
		$controller_line = '';
		// Get your source in Controller (all contents)
		if ($fp = fopen('./application/libraries'. '/' . $file_name . '.php', "r")){
	...
	public function my_doc_lib_detail($file_name)
	{
		$current_url = explode('/docs', current_url());
		$api_list = array();

		$txt = $this->_get_lib_source($file_name);
	...
	public function pdf_lib()
	{
		...
		$data['google'] = $this->my_doc_lib_detail('google');

		$this->load->view('pdfreport_lib',$data);
	}
}
